...
* 每条一行：路径 +「什么时候该读」，覆盖改/新建/评审/排查；不要只写「它是什么」。
...
* **强度规则覆盖**：项目根导航、`_standards/*.md`、`workspace-docs/*/README.md`、全局指令导航--凡实际入口均须「何时该读 + 必读 + 后果」。
* **新写文档立即登记并反向核对**，禁止用待办占位。
* 指针尽量就近贴在支撑的那条规则旁；底部导航作兜底全集。
...
**收工复盘（兜底）**：本会话有可复用发现或代码变动导致文档失效时，**必须调用 `doc-update` skill**；若判定无需更新，按其「本次无需更新」收工。第一触发点是结论产生的那一刻：任何花搜索/调查/多轮试错换来的可复用结论（根因+解法、选型裁定、同类工具行为、官方行为变化等），拿到即落盘，不等收工、不等用户确认；未验证的标「待用户确认」。本条只接漏网之鱼。
"""
