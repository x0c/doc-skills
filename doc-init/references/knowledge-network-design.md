...
2. 领域知识库是主干：每个**业务领域 / 业务条线**生成一份 `*_KNOWLEDGE_BASE.md`，内部包含该领域从配置到执行的**完整链路**——业务逻辑、代码入口（跨多个模块）、表入口、流程入口、隐性知识和验证路径。一份 KB 应能回答"改这个业务需要知道什么"的全部问题，而不是让 AI 到另一份"模块 KB"里找另一半。
...
doc-init 产出的定位是「可用级初稿」——足以支撑 Agent 独立完成中等复杂度任务，但不是经验级完成品。真正的血肉（真实踩坑、线上故障、精确排查 SOP）来自后续日常开发中的 doc-update 增量沉淀。

**doc-init 的交付标准**：
- Agent 读完文档后能正确定位到代码入口（§3/§4/§5 非空）
- Agent 知道不能做什么（§6 有 AI 易错点标记的隐性约束）
- Agent 改完后知道怎么初步验证（§7 有验证路径，哪怕是模板级的）
- Agent 知道相关但不在本域的知识去哪里找（§8 关联文档）

**每个 KB 的 §9 必须明确区分置信度层级**：

| 层级 | 来源 | 示例 |
|------|------|------|
| 高置信 | 从代码/测试/配置直接推导 | 状态枚举值、表结构、接口路径、注解约束 |
| 中置信 | 从代码模式推断但未运行验证 | 状态转换方向（看到 transition 方法但没跑过）、配置生效条件 |
| 低置信/待补充 | 需要用户经验或运行验证 | "为什么用这个字段而不是那个"、异常处理策略、性能边界 |

**严禁伪造经验**：doc-init 不会伪造排障记录、线上故障、性能数据。看不到的就标「待补充」，不要编造"常见问题"。depth_scanner.py 提取到的模式只是候选信号，必须由模型确认后才写入 KB 正文。
