...
**多组件 monorepo（根无 `docs/`、文档落在子仓）**：先找含 `docs/` 或 `specs/` 且含 `AGENTS.md` 的子目录（跳过 `worktrees` / `node_modules` / `.git`），对**每个**文档宿主分别跑 `audit.py`；根只做 CLAUDE/`AGENTS` 体量与导航指针检查。禁止只审仓库根却漏掉 `backend/docs`、`client-web/docs` 等。

运行 `python3 scripts/audit.py [项目根或子仓根]`，一次跑完机器可判的检查：
...
**事故文不在 `troubleshooting/` 时仍算台账（AlphaForge 2026-09-05）**：许多仓把事故写在 `docs/operations/`（`*incident*` / `*fix*` / `YYYY-MM-DD-*.md`）。`audit.py` G 项把 operations 事故候选与 troubleshooting 合计计入门槛——**人工仍按类型驱动折叠**。优先建**指针型** `TROUBLESHOOTING_INDEX.md`（症状→权威原文路径），**不要默认搬迁文件**（搬迁=高风险全仓引用同步）。原文继续留在 `operations/`，索引写清「权威源在原文、索引可跳过条件」。
...
- **索引重建**：只列真实文档，按领域聚类、高频在前；删死链/空占位；受保护段跳过；J 列出的🔒托管块原样保留
- **注入块/裸索引**：① 读块内容识别规则；② 对照 AGENTS.md 逐条判断是否已覆盖；③ 未覆盖的提炼后并入；④ 删整个注入块
- **`managed:inherited-agents`（Codes + 本机 `sync-agent-files`）**：**禁止**当「工具注入块残留」删掉——`pre-commit` 会在下次提交整份回注。死链修法是改**产品根**导航为绝对路径（见全局 `docs/WORKSPACE_ORGANIZATION_GUIDE.md`），不是清块。审计 C 项若列出该块，记为预期机制（C 项已豁免该标记；J 会把它列入🔒托管块）。
- **`managed:inherited-agents`（无回注 hook 的仓）**：子仓专属规则已覆盖父仓裁定时，才可删注入块避免双源；删前确认硬裁定仍在子仓必读入口可达。禁止把父仓路径（`backend/docs/...`）原样留在子仓正文里变成死链。
...
- 主 agent 默认不直接压文档正文，逐篇压缩分发 subagent 并行做；主 agent 上下文只装「分片方案 + 判据清单 + 回传账目」。分片失败时的降级见下「subagent 模型与额度失败回退」
- subagent 必须注入三样：① compression-guide.md 全文（判据基准）；② 本项目受保护段清单（根 AGENTS.md「领域地图」「待补充知识库」段、KB 的 `§0 目录索引` / `§1.5 架构概览` mermaid / `§2.5 物理路径速查`、方法名锚定引用、J 列出的🔒托管块——不可删不可压）；③ 高风险清单（即下方「先列清单确认」五项，subagent 不得擅自执行，回传主 agent 裁定）。缺这三样，多 subagent 并行压出来的判据漂移比单 agent 串行更严重
- subagent 只动 `docs/`（及本仓 `specs/`）下自己分到的文档，不碰根 AGENTS.md / 导航 / 索引——索引重建与全仓引用同步是全局视角，主 agent 在 Step 4 串行做
...
```bash
# 必须排除 .worktrees / 构建缓存，勿把 worktree 副本算进产品 docs（脚本已 prune）
...
**系数来源（实测，非估计）**：mc-mdcrm 仓库 14 篇文档抽样，`tiktoken cl100k_base` 加权平均系数 = **0.47**（叙述密集 0.55~0.60，代码/表格密集 0.29~0.45）。细节见 `plan_shards.py` 头注释。
...
**subagent 模型与额度失败回退（2026-09-05）**：派 Task 时**默认 `model` 继承父会话**，禁止为提速写死易耗尽额度的 slug（曾因指定 `composer-2.5` 整批 `Increase limits` 失败）。若并行 Task 全部起不来：① 立刻改默认模型重试一小片冒烟；② 仍失败则主 agent **降级串行**，优先事故/排障 → 研究评估 → 已密 KB/规格「审后标无可压」；③ 收工报告必须写明「分片失败已降级」，禁止假装并行已跑完。

**二轮再压常态**：距上次压缩 >30 天的架构 KB / 规格 / playbook，大量「已审无可压」是**合格结论**，不是偷懒。禁止为凑压缩比去删字段表、curl、阈值。事故文仍应用排障力度尺，不能与 KB 同一把尺。
...
## Step 6.5 — 跨项目经验抽升全局（不可跳过）

压缩与审计通过后，**扫本产品 `docs/`（及根 AGENTS 中的跨切面段落）**，找出换到别的产品仓仍成立的经验：

| 类型 | 典型落点（agentsync 真身） |
|------|---------------------------|
| macOS 菜单栏 / 登录静默 / 隐藏图标 | `docs/MACOS_APP_DEVELOPMENT_GUIDE.md` |
| 系统权限 / 自动化 / Apple Events | `_standards/.../macos-system-permissions.md` 或全局索引已登记专项 |
| 分发 / 公证 / Sparkle / Developer ID | `docs/APP_STORE_CHINA_LISTING_GUIDE.md` 或既有分发指南 |
| 本机代理 / Claude 入口 | `docs/MAC_PROXY_AGENT_GUIDE.md` |
| 工作区 / inherited 注入 / 无 `.git` | `docs/WORKSPACE_ORGANIZATION_GUIDE.md` |
| 泄漏门禁 | `docs/LEAK_GATE_GUIDE.md` |
| 其它跨产品机制 | 新建或增补对应 `docs/*_GUIDE.md`，并在全局 `AGENTS.md` 规则索引补触发词 |

**动作**：与全局权威对照——全局缺则写入/增补；产品仓把通用段改成「权威见全局 …；本产品专有：…」指针，禁止只把可复用段压短后仍独占在产品仓。与全局矛盾时不擅自裁定，列入收工「待用户确认」。

**本步不算可选**：用户 2026-09-05 裁定文档治理必须主动抽升。收工报告须单列「抽升全局」账目（写了哪篇 / 已有故跳过 / 待确认）。
...
| [`scripts/audit.py`](scripts/audit.py) | Step 2 / Step 6 自动审计脚本（含检查 J 与 metrics 基线对比） |
