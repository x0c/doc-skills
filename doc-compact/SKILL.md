...
运行 `python3 <DOC_INIT_DIR>/scripts/insert_doc_governance.py <全局文件路径>`。
`[跳过]` = 已最新；`[新增/升级]` = 已写入，扫一遍其余章节删旧约定。
`<DOC_INIT_DIR>` 不存在时：逐条对照 `references/standard.md` 手工比对，报告中注明「未自动校验」。

## Step 2 — 只读审计

运行 `scripts/audit.sh [项目根]`，一次跑完机器可判的检查：
...
**执行分级**：
- **低风险，直接做**：复述 / 死链 / 历史沿革 / 空占位 / 重复提醒（汇报列出即可）
- **高风险，先列清单确认**：删整篇、成段重写、拆并索引结构、删改含数字/边界条件正文
...
```bash
scripts/audit.sh <项目根> --compact-date <今日 YYYY-MM-DD>
```
...
- **工具注入会回来**：gitnexus 等工具重新分析时可能写回注入块，删了就再删；治本要在工具侧关闭注入选项
...
| [`scripts/audit.sh`](scripts/audit.sh) | Step 2 / Step 6 自动审计脚本 |
