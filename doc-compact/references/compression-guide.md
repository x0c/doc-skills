...
SKILL.md Step 5「分片执行协议」把逐篇压缩分发给多个 subagent 并行执行。这放大了上述漂移风险：单 agent 串行压多次是**同一判据理解在不同时间漂移**，多 subagent 并行是**不同判据理解在同一时间发散**——后者更隐蔽，因为各 subagent 独立返回看似合理的账目，主 agent 若不交叉比对就发现不了跨片不一致。
