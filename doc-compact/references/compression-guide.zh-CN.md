...
4. **压缩标识是否齐全**：各 subagent 压完应自行追加 `<!-- 该文档整理/压缩于 YYYY-MM-DD -->`，但并行场景容易漏。主 agent 汇总后跑 Step 6 的 `audit.py --compact-date` 硬闸门兜底——摘要行 `missing compact stamp=0` 是并行场景的最后一道防线，任何缺标识 = 该 subagent 漏标，补完才算闭环。若多数文档因「近 30 天已压」被跳过，真实闸门为「今日标识 ∪ 近 30 天标记 = 全集」。
